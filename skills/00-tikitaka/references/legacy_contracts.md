...
Every timed middle-caption row that con
tinues to CapCut must carry a reference
...
`--lo
ad-extension`, checks for the extension content-script marker, and if Chrome
...
the compl
ete Stage 1 production design without reading implementation labels.
...
F    | instagram_card_tts       | 인스타/커뮤니티 카드형            | card/comment/community/story asset + TTS/B
GM
```
...
For example, if the source only proves "a soldier met his
 daughter at a
