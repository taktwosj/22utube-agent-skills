...
def validate_source_voice_separation(
    ...
    locked_source_duration = require_number(lock, "duration_sec")
    if locked_source_duration <= 0:
        raise GateFail(
            "WAIT_SOURCE_VOICE_DURATION_PARITY: locked source duration must be positive"
        )
