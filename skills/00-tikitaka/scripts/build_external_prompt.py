"""Build an external review prompt packet for the Shorts lane.

Thin lane adapter over the shared prompt factory. Reads the named template
from skills/00-tikitaka/prompt_templates/, the named input artifacts, and
the shared contracts, then writes:

    <episode-root>/20_script/external_packets/<packet>.md
    <episode-root>/20_script/external_packets/<packet>.manifest.json

The packet is generated locally only. It does NOT open ChatGPT, call an
API, or transport a packet (NORM-005: manual external transport).
"""
...
SKILL_TEMPLATES = REPO_ROOT / "skills" / "00-tikitaka" / "prompt_templates"
...
def _import_prompt_factory():
    ...
    spec = importlib.util.spec_from_file_location(
        "tikitaka_prompt_factory", SHARED_CORE
    )
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    sys.modules["tikitaka_prompt_factory"] = mod
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a Shorts external review prompt packet."
    )
