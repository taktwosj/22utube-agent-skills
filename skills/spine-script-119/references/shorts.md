...
사용자가 그 회차에서 편수·길이를 더 요구하면 `mark_shorts.py --max-shorts 4 --max-len 120` 처럼 명시 인자로만 푼다(기본값은 계약 3편·90초).

**기본 경로는 `gen_short_hf.py`** — cards_def.SHORTS 의 `hf_head`/`hf_tail` 비트로 세로(1080x1416) 하이퍼프레임
head·tail mp4 를 만들어 `SHORTS_ART` 에 `art`·`_tail` 이름으로 놓는다(2026-09-19, 0918 회차 코드 스킬화). 외부 생성기 삽화는 아래 대안이다.
