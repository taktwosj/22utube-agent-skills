...
BASE_CORRECTIONS = [
    ...
    ("타기수로", "타깃으로"), ("타기수", "타깃"), ("타겟", "타깃"),
    ("수사기로", "수사기록"), ("수사 기로", "수사기록"),
    ("수사 보고소", "수사보고서"), ("수사보고소", "수사보고서"),
    ("녹치록", "녹취록"), ("녹취서", "녹취록"), ("녹지록", "녹취록"),
    ("대북 소송금", "대북송금"), ("대북소송금", "대북송금"), ("대북 송금", "대북송금"),
    ("구성영장", "구속영장"), ("구속 영장", "구속영장"),
    ("기소 유예", "기소유예"), ("기소유애", "기소유예"),
    ("공소 시효", "공소시효"),
    ("면책 특권", "면책특권"),
    ("공익 제보", "공익제보"), ("공익제보자", "공익제보자"),
    ("봉지국", "봉지욱"), ("봉주 기자", "봉지욱 기자"),
    ("장 인수", "장인수"),
...
def table() -> list[tuple[str, str]]:
    """현재 교정표. 다른 단계가 '정상 표기' 집합을 만들 때 쓴다."""
    return list(_TABLE)
...
def correct(text: str) -> str:
    ...
    키가 `=` 로 시작하면 cue 문장 전체가 정확히 같을 때만 바꾼다. `렸어요`, `하.` 처럼
    짧은 조각을 부분 치환하면 다른 cue 를 망가뜨린다. 부분 치환 규칙은 두 번 걸려도
    결과가 같아야 한다 — build_assets 가 cue 에 한 번, 합친 문장에 한 번 더 건다.
    """
    for a, b in _TABLE:
        if a.startswith("="):
            if text.strip() == a[1:]:
