...
그리고 [template-contract.md](references/template-contract.md)를 읽는다. 잠긴 v1을
배포하거나 복제할 때는 [template-v1-handoff.md](references/template-v1-handoff.md)도
읽는다. 설치된
...
template_git_bundle={skill_root}\assets\templates\politics-longform-template-v1
template_default={factory_root}\02_politics_longform\templates\politics-longform-template-v1
episode_project={episode}\60_hyperframes\project
```

Git 번들을 템플릿 원본으로 사용하고 factory의 공용 템플릿 경로에는 이를 복제해
배포한다. 공용 템플릿은 episode 밖에 둔다. episode 프로젝트는 공용 템플릿을
복제하거나 명시적으로 참조한다. 과거 episode와 CapCut draft를 템플릿으로
승격하지 않는다.
...
2. 잠긴 v1이 필요하면 Git 번들의 `template_lock.json` 해시를 먼저 검증한다.
3. factory 공용 템플릿은 검증된 Git 번들에서 복제하며 임의로 재구성하지 않는다.
4. 새 버전 변경은 테스트를 먼저 작성하고 RED를 확인한다.
5. `style_tokens.json`을 단일 조절값 권위로 사용한다.
