...
- `timeline_design` segment missing source_ref/source_order/timeline_order/assembly_role
- `timeline_design` segment missing visible_text_role/audio_role/duration_basis/duration_status/visual_strategy
...
- missing `20_script/tts_copy_text.txt` when any `caption_type=tts_narration`
  exists
- narration-audio segment exists but `20_script/tts_duration_probe.json` is missing
- narration-audio segment exists but `20_script/tts_timing_reconciliation_gate.json` is missing
- `tts_caption/audio_role=none` incorrectly requires TTS timing files
- `tts_caption/audio_role=none` triggers narration audio generation or request
- source_order is treated as timeline_order
- timeline_order is regenerated by `000short-production-agent`
- assembly_role sequence changes in `capcut_layout_plan.json` or
  `capcut_timeline_manifest.json`
- duration_basis changes without `WAIT_TIKITAKA_DESIGN_REPAIR`
- duration_status changes without `WAIT_TIKITAKA_DESIGN_REPAIR`
