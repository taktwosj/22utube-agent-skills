...
- Current Instagram template master is `260625-ig-contortion-top3-urakkai-instagram-tts` with `catcup_reference_layout_profile="ig_contortion_top3_instagram_tts_template_master_v1"`. Copy the whole draft folder first, preserve `subdraft`, `Resources/combination`, preset audio placeholder relationships, sticker/effect rows, track count, and z-order, then replace only source/text/timing/audio values.
...
- Current CatCup placement profile: `broadcast_accident_instagram_media_v1`.
...
- Treat `sfx_manifest.json` as the source of truth for available SFX files and tags.
...
- Instagram/Reels `인스타템플릿` means the template master `260625-ig-contortion-top3-urakkai-instagram-tts`, described by Git manifest `manifests/capcut-template-set.json` and stored as a OneDrive/CapCut draft bundle. Settings JSON is a replacement-value list only; never build a lookalike project from `source.mp4 + PNG + text`.
- Instagram/Reels creation must copy the master draft folder first and preserve `draft_content.json`, `draft_meta_info.json`, `draft_virtual_store.json`, `subdraft`, `Resources/combination`, `materials.drafts`, preset audio placeholder relationships, sticker/effect rows, 12 total tracks, 6 editable text tracks, and z-order. Then replace only current job `source.mp4`, visible text, timing, and TTS/audio values.
...
- The video material inside a selected template may be placeholder media. Do not copy placeholder media into the active generated draft; after cloning the template, strip placeholder video/audio/nested draft/subdraft/stale metadata and add only real source/project media.
