#!/usr/bin/env python3
"""Validate the explicit top5isu_v1 track and coordinate adapter."""
...
def validate_top5isu_track_mapping(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not truthy(data.get("derived_from_reference_project")):
        raise GateFail(
            "FAIL_TOP5ISU_REFERENCE_NOT_CLONED: derived_from_reference_project must be true"
        )

    if data.get("required_tracks") != EXPECTED_TRACKS:
        raise GateFail(
            "FAIL_TOP5ISU_TRACK_MAPPING: required_tracks must be "
            f"{EXPECTED_TRACKS!r}"
        )
