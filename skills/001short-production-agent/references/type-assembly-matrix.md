...
## 나레이션은 전 구간을 채운다 (2026-08-22 길이 여유만 완화)

생성 A9는 영상 길이 전체를 공백 없이 덮는 것이 원칙이나, 정확히 채워야 하는 강제는 아니다.
**나레이션 총 길이는 영상 총 길이 − 3초까지 허용한다**(예: 영상 50초면 나레이션 47초 이상이면 PASS).

자막 cue를 V 세그먼트 2개에 걸치는 배치는 **여전히 금지**다. `validate_audio_caption.py`의
`_authoritative_mapping`이 V열 하나당 매핑 하나를 강제 생성하고 evidence의 mapping이 그 집합과
완전히 같아야 통과하므로(`CAPTION_TIMING_BUILD_MAPPING_MISMATCH`), cue가 V 경계를 넘으면
검증기에서 무조건 떨어진다. 2026-08-22에 이 문서에 "V 2개 걸침 허용"을 잠깐 적었으나 검증기가
지원하지 않는 걸 뒤늦게 확인해 되돌렸다 — **V 세그먼트마다 한 컷씩** 배치가 여전히 유일한 유효
경로다. 문장 길이는 구간 길이 × 약 7자/초로 잡고, 생성 후 실측이 구간을 넘으면 문장을 줄여 다시
생성한다. 앞뒤 무음은 `apad`로 구간 길이에 정확히 맞춘다. VIDEO·A9·A9_TEXT의 duration은 실측
오디오 길이가 아니라 **V 구간 범위**로 바인딩한다(실측값을 쓰면 µs 단위로 구간을 넘어 cue 포함
검사에 걸린다).
