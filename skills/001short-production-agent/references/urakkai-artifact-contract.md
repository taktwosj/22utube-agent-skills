...
`shrt_white_base_v3` 시드는 자리표시자라 폭·줄바꿈 설정이 없고, 빌더는 문자열만 갈아끼우며
...
## 숫자 계약은 하나로 정한다 (260817 개정)

파일마다 정밀도와 상한이 달라 조립이 반복해서 막혔다. 값을 하나로 모았다.

### 시간 — 1프레임 허용오차

같은 경계를 표는 소수점 3자리 글자로, 미디어는 프레임 번호로, SRT는 밀리초로 적는다.
셋이 수백 µs씩 다른 것은 **정상이다.** 그래서 파일 간 시간 비교는 완전일치가 아니라
`common.FRAME_TOLERANCE_US`(34ms, 30fps 한 프레임) 안이면 같은 것으로 본다.

- `times_match(a, b)` / `ranges_match(a, b)`를 쓴다. `!=`로 직접 비교하지 마라.
- 34ms는 사람이 인지하지 못한다. 이 오차로 결과물이 어긋나 보이는 일은 없다.
- 표 머리글은 여전히 소수점 3자리로 쓴다. 정밀도를 늘려서 맞추려 하지 마라.

허용오차가 적용되는 곳과 안 되는 곳을 갈라두면 판단이 쉽다.

| 허용오차 (±1프레임) | 완전일치 |
|---|---|
| 구간 시작·끝·길이 | 자막 **문구** |
| 클립 소스·타깃 범위 | 파일 **SHA-256** |
| 자막 cue ↔ SRT 시각 | `segment_id` · `cue_id` · `role` |
| 표 ↔ 타임라인 시각 | `schema_version` 상수 |
| 빌더 산출물 ↔ 승인 타임라인 | 화자 배정 |

가르는 기준은 하나다. **틀을 벗어나면 완전일치, 틀 안에서 흔들리는 것은 허용.**
34ms 어긋난 경계는 결과물이 같지만, 글자 하나 다른 자막이나 해시가 다른 파일은 다른 결과물이다.

### 글자수 — 두 개뿐

| 대상 | 상한 |
|---|---|
| 자막 계열 `A9_TEXT` · `A10_TEXT_*` · `STATE_LASER` | 한 줄 **15자**, 최대 2줄 |
| 제목 `T1` · `T2` | 각 한 줄 **12자**, 줄바꿈 없음 |

이전에는 A9_TEXT가 문맥에 따라 10자 또는 15자, STATE가 8자 또는 15자, T1이 10자로
문서마다 달랐다. 전부 위 두 값으로 통일했다.
