...
8. 새 A9 TTS와 짝인 target `A9_TEXT`도 2줄, 한 줄 15자 이하로 쓴다(`protocol.json`의 `target_a9_text_max_chars_per_line`, validator와 동일). 원본 `A9_TEXT`와 `STATE_LASER`의 기존 한 줄 15자 계약도 동일하게 유지한다.
