...
| 05 | `workflow.json`; episode state; `steps/05-final-blueprint.md`; `references/checks/design-lock.md` | `references/root-contract-production-plan.md` when compiling the v2 plan; `A12_RESERVED_EMPTY` is validation-only and never a placement | Every other `steps/`; `protocol.json` prose; `references/**` glob |
