...
- The Stage 04 review judges creative urakkai against `references/mara-urakkai-review-rubric.md`: MARA_MESSAGE, narrative_arc, independent narrative, paraphrase test, 야부리 VIDEO reorder, wow point, A9 narration, T1/T2/A9_TEXT/STATE, and audio policy. `narrative_arc` is the story shape; 가단야 is the production process (가 guideline / 단 wording change / 야 wow-point reorder) defined in `references/shorts-structure-taxonomy.md`. Never use 가단야 for the story shape. It returns exactly one verdict — `PASS_CANDIDATE`, `REVISE_REQUIRED`, `WAIT_SOURCE_RECHECK`, or `REJECTED_MARA_INSUFFICIENT` — and no verdict advances the episode. Review evidence without a verdict fails with `URAKKAI_REVIEW_RUBRIC_MISSING`.
- Invention is allowed. Keep `SOURCE_OBSERVATION`, `CREATIVE_URAKKAI`, and `FICTIONAL_RECONSTRUCTION` separate; an invented relationship, motive, or identity recorded as source fact fails with `URAKKAI_FACT_BOUNDARY_VIOLATION`. Never deduct for invention alone.
- Every urakkai design declares one audio policy, `TTS_ONLY_MUTE_SOURCE` or `A10_RETAINED_SYNC`. Mixing original voice with new TTS without a verified vocal stem, or declaring neither, fails with `URAKKAI_AUDIO_POLICY_UNDECLARED`.
