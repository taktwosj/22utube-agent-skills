...
- The Stage 04 review judges creative urakkai against `references/mara-urakkai-review-rubric.md`: MARA_MESSAGE, 가단야 structure, independent narrative, paraphrase test, VIDEO reorder, wow point, A9 narration, T1/T2/A9_TEXT/STATE, and audio policy. It returns exactly one verdict — `PASS_CANDIDATE`, `REVISE_REQUIRED`, `WAIT_SOURCE_RECHECK`, or `REJECTED_MARA_INSUFFICIENT` — and no verdict advances the episode. Review evidence without a verdict fails with `URAKKAI_REVIEW_RUBRIC_MISSING`.
