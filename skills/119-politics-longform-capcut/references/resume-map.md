...
|---|---|---|
| PRE-119 강한 표식 1개 또는 보조 표식 2개 이상 | `pre119-handoff-contract.md` | 실패한 validation 또는 post-A/B/C/D join 하나 |
| 최종 대본 미승인 | `direct-script.md` | 대본과 사용자 승인 |
| 필요한 source/SRT/cut이 없거나 download·ffprobe 실패 | `source-media.md` | 실패 source 하나 |
| B가 명시 요청되어 활성이고 narration media/SRT가 없거나 API·정렬 실패 | `narration.md` | 실패 문장·파일 |
| C가 명시 요청되어 활성이고 필수 이미지가 없거나 자산 검사 실패 | `visual-assets.md` | 실패 자산 하나 |
...
실제 source media, captions, `episode_cards.json`, build report, relink readback이 재개점이다.
활성 B/C가 있으면 해당 narration media/SRT 또는 Resources도 재개점으로 쓴다. 새 상태 파일,
receipt, checkpoint, 비공식 schema를 만들지 않는다.
완료된 산출물의 hash·duration·검사 결과가 그대로면 앞 단계를 다시 읽거나 실행하지 않는다.

요청되지 않은 B/C의 파일이 없다는 이유로 해당 reference를 선택하지 않는다. 나중에 B/C가
명시 요청되면 성공한 A/D를 유지하고 새로 활성화된 작업만 시작한다.
