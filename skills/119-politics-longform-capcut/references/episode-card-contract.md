...
```text
INTRO             optional 5-second hook text over the root intro background
...
- Without an intro request, `C001` is `SOURCE_VIDEO` and starts at zero. If an
  intro is explicitly requested, `C001` is `INTRO` and is exactly 5,000,000 microseconds.
